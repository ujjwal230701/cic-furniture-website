        ...
        <div style={S.container}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 48, marginBottom: 48 }}>
            <div>
              ...
              <p style={S.body}>To provide Indian businesses with furniture that combines international design standards with local practicality — at honest, transparent prices.</p>
            </div>
            <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 1, background: "#e8e8e8" }}>
            {STATS.map(([num, label]) => (
              <div key={label} style={{ background: "#f5f5f0", padding: 32, textAlign: "center" }}>
                <div style={{ fontSize: 36, fontWeight: 800 }}>{num}</div>
                <div style={{ fontSize: 10, letterSpacing: 2, color: "#888", marginTop: 6 }}>{label.toUpperCase()}</div>
              </div>
            ))}
          </div>
        </div>
