    ...
    <div style={{ ...S.section }}>
      <div style={S.container}>
        <div style={{ marginBottom: 40 }}>
          ...
          <p style={{ color: "#666", fontSize: 13 }}>All prices exclusive of GST · EMI available on orders above ₹10,000</p>
        </div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 36 }}>
          {CATEGORIES.map(c => (
            <button key={c} onClick={() => setFilterCat(c)} style={{ padding: "8px 16px", border: "1px solid", borderColor: filterCat === c ? "#1a1a1a" : "#ddd", background: filterCat === c ? "#1a1a1a" : "#fff", color: filterCat === c ? "#fff" : "#444", fontSize: 10, fontWeight: 700, letterSpacing: 1.5, cursor: "pointer" }}>
              {c.toUpperCase()}
            </button>
          ))}
        </div>
